...
if [ -f $INPUT_FILE ] ; then
    ./$BUILD_DIR/run $INPUT_FILE < $INPUT > current_out 2> current_err || CURRENT_EXIT=$?
else
    ./$BUILD_DIR//run < $INPUT > current_out 2> current_err || CURRENT_EXIT=$?
fi
